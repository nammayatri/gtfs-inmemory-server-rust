...
<!-- ===== section 9 begins: dashboard round 4 (UX). Self-contained; sections 2, 3 and 8 are edited elsewhere. ===== -->
## 9. Dashboard requirements added 2026-09-17, round 4 (reviewer UX)

Dashboard only (`editor-ui/`), apart from the read endpoints and the merge action
in 9.5, which the API provides. Every item has checks in `dev/ui_smoke.mjs`
...
2. **A station once, not each of its platforms.** Where a panel lists stops near
   ...
   shared its point" on a coordinate review, the same-named stops of 9.5, the
...
6. **Stops sharing a point.** Stops with exactly the same coordinate (about a
   ...
   pin). The chooser counts what is shown (9.1), and differently named stops on
