...
# tests/editor_stop_details_flow.rs, tests/editor_feed_lock_flow.rs and
...
EDITOR_TEST_DATABASE_URL="$DB_URL" cargo test --locked --test editor_flow --test editor_create_flow \
  ...
  --test editor_stop_details_flow --test editor_feed_lock_flow --test editor_route_review_flow \
  -- --nocapture || status=$?
