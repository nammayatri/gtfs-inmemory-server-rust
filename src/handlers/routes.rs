...
pub fn create_router(app_state: AppState) -> Router {
    Router::new()
        ...
        .route("/route-stop-mapping/:gtfs_id/route/:route_code", get(get_route_stop_mapping_by_route))
        .route("/route-stop-mapping/:gtfs_id/stop/:stop_code", get(get_route_stop_mapping_by_stop))
        ...
        .route("/station-children/:gtfs_id/:stop_code", get(get_station_children))
...
async fn get_route(
    ...
    let route = app_state.gtfs_service.get_route(&gtfs_id, &route_id).await?;
    Ok(Json(route))
}
...
async fn get_route_stop_mapping_by_route(
    ...
    let mappings = app_state.gtfs_service.get_route_stop_mapping_by_route(&gtfs_id, &route_code).await?;
    let max_sequence_mappings = get_max_sequence_route_stop_mapping(mappings);
    Ok(Json(max_sequence_mappings))
}

fn get_max_sequence_route_stop_mapping(all_mappings: Vec<RouteStopMapping>) -> Vec<RouteStopMapping> {
...
async fn get_route_stop_mapping_by_stop(
    ...
    let mappings = app_state.gtfs_service.get_route_stop_mapping_by_stop(&gtfs_id, &stop_code).await?;
    Ok(Json(mappings))
}
...
async fn get_routes_fuzzy(
    ...
    for route in routes {
        let matches = route.long_name.as_ref().map(|n| n.to_lowercase().contains(&query_lower)).unwrap_or(false)
            || route.short_name.as_ref().map(|n| n.to_lowercase().contains(&query_lower)).unwrap_or(false)
...
async fn get_stop(
    ...
    let stop = app_state.gtfs_service.get_stop(&gtfs_id, &stop_code).await?;
    Ok(Json(stop))
}
...
async fn get_station_children(
    ...
    let children = app_state.gtfs_service.get_station_children(&gtfs_id, &stop_code).await?;
    Ok(Json(children))
}
...
async fn readiness_probe(
    ...
    if !app_state.gtfs_service.is_ready().await {
        return Err(AppError::NotReady("Service not ready - still loading initial data".to_string()));
    }
...
async fn get_service_type_by_vehicle(
    ...
    let vehicle_data = app_state.db_vehicle_reader.get_vehicle_data(&vehicle_no).await?;
