...
impl VehicleDataReader for DBVehicleReader {
    ...
    async fn get_depot_info(&self, gtfs_id: &str, entity_id: i64) -> AppResult<Option<DepotInfo>> {
        ...
        //
        // Source mirrors the per-feed allowlists in operator::{EXTERNAL_ONLY_GTFS_IDS,
        // INTERNAL_ONLY_GTFS_IDS}: external `entities` for anything not internal-only,
        // `entities_internal` otherwise.
        ...
        let result: Result<Option<(String, Option<String>)>, sqlx::Error> = if fetch_external {
