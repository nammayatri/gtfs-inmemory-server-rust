...
impl DBVehicleReaderInternal {
    ...
    /// Deliberately uncached and straight from Postgres: this backs the editing screen, where
    /// ops must see the row they just saved, not a copy up to the cache TTL old.
    async fn list_station_etas_impl(
        &self,
        gtfs_id: &str,
        variant_id: Option<&str>,
    ) -> AppResult<Vec<StationEtaRow>> {
