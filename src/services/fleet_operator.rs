...
impl DBFleetOperatorService {
    ...
    async fn login_mobile_number(
        ...
        // Source selection is driven by the per-feed allowlists in
        // operator::{EXTERNAL_ONLY_GTFS_IDS, INTERNAL_ONLY_GTFS_IDS}. Feeds that
        // can land in both (the default) try external first and fall back to
        // internal so chennai_bus continues to resolve from the primary
        // `employees` table when present.
        let fetch_external = !crate::services::operator::INTERNAL_ONLY_GTFS_IDS.contains(&gtfs_id);
        let fetch_internal = !crate::services::operator::EXTERNAL_ONLY_GTFS_IDS.contains(&gtfs_id);

        let row = if fetch_external {
            match self
                .employee_reader
...
mod tests {
    ...
    // Feed that lives in both DBs (neither EXTERNAL_ONLY nor INTERNAL_ONLY) —
    // exercises the "external first, internal fallback" branch of login.
    const EXTERNAL_FEED_GTFS_ID: &str = "chennai_bus";
    use async_trait::async_trait;
    ...
    #[tokio::test]
    async fn external_hit_short_circuits_internal_fallback() {
    ...
    #[tokio::test]
    async fn external_miss_falls_through_to_internal() {
        let (emp_arc, svc) = build_service(
    ...
    #[tokio::test]
    async fn internal_only_feed_skips_external() {
        let (emp_arc, svc) = build_service(
            StubEmployeeReader::new(
