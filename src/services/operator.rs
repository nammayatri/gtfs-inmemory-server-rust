...
impl OperatorService for DBOperatorService {
    ...
    async fn upsert_one_row(
        ...
        // Check if we're updating (PK present) or inserting (PK not present)
        let is_update = first_obj.contains_key(pk);
        ...
        // Build SQL based on whether we're updating or inserting
        let sql = if is_update {
            ...
            format!(
                "INSERT INTO public.{} ({}) VALUES {} RETURNING row_to_json({}) AS result",
                table,
                cols.join(", "),
                placeholders.join(", "),
                table
